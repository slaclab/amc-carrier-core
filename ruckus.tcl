...
if { [info exists ::env(OVERRIDE_SUBMODULE_LOCKS)] != 1 || $::env(OVERRIDE_SUBMODULE_LOCKS) == 0 } {
   if { [SubmoduleCheck {lcls-timing-core} {2.0.1} "mustBeExact" ] < 0 } {exit -1}
   if { [SubmoduleCheck {ruckus}           {2.7.2} "mustBeExact" ] < 0 } {exit -1}
   if { [SubmoduleCheck {surf}             {2.9.0} "mustBeExact" ] < 0 } {exit -1}
} else {
   puts "\n\n*********************************************************"
